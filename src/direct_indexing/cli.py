...
def cmd_run(args, config: AppConfig) -> int:
    ...
    if results:
        ...
        for r in results:
            ...
            print(f"  {status} {r.symbol}: -${r.loss_amount:.2f} ({r.loss_percent:.1f}%)")
    else:
...
def cmd_scan(args, config: AppConfig) -> int:
    ...
    for i, pos in enumerate(harvestable, 1):
        ...
        print(f"   Qty: {pos.qty} | Entry: ${pos.avg_entry_price:.2f} | Current: ${pos.current_price:.2f}")
        print(f"   Loss: ${abs(pos.loss_amount):.2f} ({abs(pos.loss_percent):.1f}%)")
        print()
...
def cmd_status(args, config: AppConfig) -> int:
    ...
    for pos in positions:
        ...
        print(f"  P/L: {pl_char}${pos.unrealized_pl:.2f} ({pl_char}{pos.unrealized_plpc:.2f}%)")
    ...
    if args.format == "json":
        ...
        if summary["top_losses"]:
            ...
            for loss in summary["top_losses"]:
                print(f"  {loss['symbol']}: -${loss['loss_amount']:.2f} ({loss['loss_percent']:.1f}%)")
